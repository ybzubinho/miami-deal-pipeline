...
# ─── HTTP SERVER ──────────────────────────────────────────────
# After scraping, serve deals.json so the dashboard can fetch it.
# This also keeps the container alive so Railway doesn't shut it down.
...
def start_server():
    ...
    print(f"  Server running on port {port} — dashboard can fetch /deals.json")
